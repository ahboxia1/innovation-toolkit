...
**传统创新流程：** 人做研究 → 人写文档 → 人做PPT → 人汇报 → 人被质疑。

**AI native流程：** 每一步都跟AI协同——不是最后让AI润色一下，而是从第一天就跟AI一起想、一起写、一起迭代。

这个工具包的核心定位：**帮员工从模糊想法走到可立项的一页纸。**

做prototype（原型/产品）和test（用户验证/市场测试）不是这个工具包的事——那需要根据具体场景开发数字员工、做产品、找用户验证。本工具包只做前三步：搞懂战场、定义问题、想出方案。
...
> *"Empathy is the foundation of human-centered design." — d.school Bootleg*
...
> *"The define mode is when you unpack your empathy findings into needs and insights and scope a meaningful challenge." — d.school Bootleg*

**做什么：** 把调研结果/模糊信息拆解成**needs（需求）和insights（洞察）**，形成POV（Point of View）——一个清晰的问题陈述。
...
2. 提炼needs和insights
3. 形成POV
4. 从POV推导出"How Might We"问题（发散方案的起点）
5. 触发**方向验证gate**
...
> *"Ideate is the mode in which you generate radical design alternatives — a mode of 'flaring' instead of 'focus'." — d.school Bootleg*

**做什么：** 基于POV和HMW问题发散方案，收敛到一张**立项一页纸**。

d.school的Ideate不是漫无目的地想idea——是**基于定义好的问题发散**。先追求数量和多样性（Go Wide），再收敛到最有潜力的方案（Narrow In）。
...
| **05-review** | 四巨头完整版评审（200-400字深度分析） | 「帮我审一下这个idea」 |
| **06-knowledge-base** | 组织创新记忆，记录历史idea | 「历史idea」「查一下做过类似的」 |
...
├── install.bat / .sh        ← 安装脚本
├── scripts/
│   └── check-env.mjs        ← 环境健康检查
│
├── 01-research/             ← Step 1: Empathize（共情 · 行业研究）
│   ├── SKILL.md
│   ├── spider.mjs              Playwright 爬虫
│   └── templates/              行业扫描 HTML 模板
│
├── 02-define/               ← Step 2: Define（定义 · 问题定义）
│   ├── SKILL.md                POV框架 + 方向验证gate
│   └── templates/              Idea卡片 HTML 模板
│
├── 03-ideate/               ← Step 3: Ideate（孵化 · 方案收敛）
│   ├── SKILL.md                DT/千丁双模板 + 段永平Hard Gate
│   ├── references/             原始制度文件
│   └── templates/              HTML 一页纸模板
│
├── 04-pitch/                ← 可选: 路演材料生成器
│   └── templates/              路演 HTML 模板
│
├── 05-review/               ← 可选: 四巨头完整版评审
│   ├── personas/               巴菲特/马斯克/纳瓦尔/Karpathy 人格文件
│   └── templates/              评审报告 HTML 模板
│
...
| 方法 | 来源 | 用在哪 |
|------|------|--------|
| 设计思维 | Stanford d.school Bootleg (2018) | 整体框架 |
| POV框架 + HMW问题 | d.school Define阶段 | Step 2 |
...
内部使用。方法论和模板来自开源项目（MIT License）和公开资料。
