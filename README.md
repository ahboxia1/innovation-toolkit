...
基于 Stanford d.school 设计思维（Design Thinking）前三阶段（Empathize → Define → Ideate），为 Claude Code 环境蒸馏。

`Design Thinking`（设计思维） `AI Native`（AI原生） `Claude Code Skill`（技能）
...
d.school的五阶段不是线性瀑布（Waterfall），而是一组可以反复进入的思考模式（Mode）。本工具包覆盖前三阶段（Empathize → Define → Ideate），后两阶段（Prototype → Test）交给外部。
...
> **非线性。** d.school强调这五步不是严格的线性流程。你可以在Define（定义）时发现需要重新Empathize（共情），在Ideate（孵化）时发现POV（观点陈述）定义得不对。本工具包支持从任意步骤进入。
...
d.school的Define不是简单地说"问题是什么"，而是形成一个独特的设计视角：你在解决谁的什么问题，为什么这个问题值得解决，什么是game-changing（颠覆性）的方向。
...
**怎么做：**
1. AI帮你从调研结果中提取关键发现
2. 提炼needs（需求）和insights（洞察）
3. 形成POV（观点陈述）
4. 从POV推导出HMW（How Might We，我们不妨怎样）问题——发散方案的起点
5. 触发**方向验证gate（关卡）**
...
或者直接跳到任意步骤：
```
帮我调研一下智能家居赛道    # → Step 1 Empathize（共情）
帮我把问题说清楚             # → Step 2 Define（定义）
帮我想方案 / 写立项一页纸    # → Step 3 Ideate（孵化）
```
...
方向验证gate（关卡）已经内嵌在Step 2中（轻量版四巨头快审），05-review是完整版。
...
innovation-toolkit/
│
├── SKILL.md                          主入口skill（技能）定义，三步流程总指引
├── README.md                         本文件
├── LICENSE                           MIT License（开源许可）
...
│   │   └── research-data-schema.json    研究数据JSON Schema（数据结构定义）
│   ├── examples/
...
│   ├── SKILL.md                         skill定义：POV框架 + 方向验证gate（关卡）
│   ├── templates/
...
│   ├── SKILL.md                         skill定义：发散收敛 + DT/千丁双模板 + 段永平Hard Gate（硬关卡）
...
│   │   ├── preview.mjs                  浏览器预览脚本
│   │   └── proposal-schema.json         立项数据JSON Schema（数据结构定义）
│   ├── examples/
...
│   │   └── karpathy.md                   Karpathy：Software 3.0、March of Nines（九的行军）、数据飞轮
│   ├── scripts/
│   │   ├── build-review.mjs             评审报告HTML构建脚本
│   │   ├── preview.mjs                  浏览器预览脚本
│   │   ├── radar-calc.mjs               四维雷达图计算
│   │   └── review-schema.json           评审数据JSON Schema（数据结构定义）
│   ├── examples/
...
└── 06-knowledge-base/                可选: 组织创新记忆
    ├── SKILL.md                         skill定义：写入/检索时机 + CLI（命令行）用法
